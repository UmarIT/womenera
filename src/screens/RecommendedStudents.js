...
const RecommendedStudents = (props)=> {
const [qualification, setQualification] = useState();
const[isLoading, setisLoading] = useState(true);
const[myUserData, setMyUserData] = useState()

const suppose='Software Engineering, Computer Science'
async function getquali (){
  ...
  const getUserData = async () =>{
    try {
        const response = await fetch( "https://nimerta123.github.io/studentapinew/studentapi.json");
        const myData = await response.json();
        setMyUserData(myData.Student);
        setisLoading(false);
        console.log("hook", myData.Student )
